...
Page({
  ...
  async getCates() {
    ...
    const msg = await myrequest("/tags","get");
    const msg1 = await myrequest("/alllist","get");
    // this.Cates = res.data.message;

    
    this.Cates = msg1.data;
    // 把接口的数据存入到本地存储中
    wx.setStorageSync("cates", { time: Date.now(), data: this.Cates });
    // 构造左侧的大菜单数据
    let leftMenuList = msg.data.newqztags;
    
    // 构造右侧的商品数据
    // let rightContent = this.Cates[0].children;
    let tongshu = msg1.data.filter(v => v.b_tag === "童书")
    let rightContent = tongshu
    this.setData({
      leftMenuList,
      rightContent
    })
  ...
  handleItemTap(e) {
    ...
    let rightContent = this.Cates.filter(v => v.b_tag === index2)
    this.setData({
      currentIndex: index1,
      rightContent,
